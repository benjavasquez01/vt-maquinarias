...
const WHATSAPP_NUMBER = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER ?? "15555550000";
...
export function FloatingWidgets() {
    ...
    try {
      await fetch("/api/lead", {
        ...
        body: JSON.stringify({
          ...
          machinesOfInterest: "",
          language: "en",
          transcript: "Exit-intent email capture",
          source: "exit-intent",
        }),
      });
    } catch { /* non-critical */ }
  ...
  return (
    <>
      ...
      <a
        href={`https://wa.me/${WHATSAPP_NUMBER}?text=Hi%2C%20I%27m%20interested%20in%20VTM%20Tech%20Solutions%20machinery.`}
        target="_blank"
        rel="noopener noreferrer"
        onClick={() => trackEvent("whatsapp_click")}
        className="fixed bottom-24 right-6 z-40 w-14 h-14 bg-[#25D366] rounded-full flex items-center justify-center shadow-lg hover:scale-110 transition-transform"
      ...
      {showExitIntent && !exitDismissed && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label="Stay in touch">
          <div
          ...
          <div className="relative bg-white max-w-md w-full shadow-2xl p-8 md:p-10">
            ...
            {exitSubmitted ? (
              <div className="text-center py-4">
                ...
                <p className="font-headline font-bold text-vtm-dark text-xl mb-2">You&apos;re on the list.</p>
                <p className="text-vtm-gray-mid text-sm">We&apos;ll send you useful content — no spam.</p>
              </div>
            ) : (
              <>
                <div className="w-10 h-1 bg-vtm-red mb-6" aria-hidden="true" />
                <h2 className="font-headline text-2xl font-bold text-vtm-dark mb-3">
                  Before you go — talk to our AI specialist
                </h2>
                <p className="text-vtm-gray-mid text-sm mb-6 leading-relaxed">
                  Our AI can answer product questions, estimate pricing, and help you figure out which machine fits your shop — in under 5 minutes.
                </p>
                <button
                  onClick={() => {
                    setShowExitIntent(false);
                ...
                <div className="relative flex items-center gap-4 mb-4">
                  ...
                  <span className="text-vtm-gray-mid text-xs">or</span>
                  <div className="flex-1 h-px bg-vtm-gray-border" />
                </div>
                <form onSubmit={handleExitSubmit}>
                  <p className="text-vtm-gray-mid text-xs mb-3">Get our machine selection guide by email:</p>
