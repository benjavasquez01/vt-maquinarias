...
export const metadata: Metadata = {
  title: "Soluciones para Equipo Gastronómico",
  description:
    "Corte láser, plegado CNC, soldadura láser y limpieza para fabricantes de equipo gastronómico en acero inoxidable: mesones, campanas y cocinas. Soporte en Chile.",
};

export default async function EquipoGastronomicoPage({
  params,
}: {
            ...
            {c.machines.map(({ name, href, spec, tag, imageId }) => (
              <Link key={href} href={href} className="group border border-vtm-gray-border hover:border-vtm-dark transition-colors">
                <div className="relative aspect-[4/3] bg-vtm-gray-light overflow-hidden">
                  <Image src={imageId} alt={name} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 768px) 100vw, 33vw" />
                </div>
                <div className="p-6">
