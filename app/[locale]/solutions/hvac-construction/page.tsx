    ...
    <>
      <section className="relative bg-vtm-dark pt-32 pb-20 overflow-hidden">
        <div className="absolute inset-0" aria-hidden="true">
          <Image src="https://images.unsplash.com/photo-1504917595217-d4dc5ebe6122?w=1600&q=80&fit=crop" alt="" fill className="object-cover opacity-15" priority sizes="100vw" />
      ...
      <section className="bg-white py-20 md:py-28">
        <div className="max-w-screen-xl mx-auto px-6 lg:px-10">
          ...
          <h2 className="font-headline text-4xl font-bold text-vtm-dark tracking-tight mb-12">{c.applications.headline}</h2>
          <div className="grid md:grid-cols-2 gap-6">
            {c.applications.items.map(({ title, body }) => (
              <div key={title} className="p-8 border border-vtm-gray-border">
                <h3 className="font-headline font-bold text-vtm-dark text-lg mb-3">{title}</h3>
        ...
        <div className="max-w-screen-xl mx-auto px-6 lg:px-10">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {c.equipment.items.map(({ name, href, desc }) => (
              <Link key={href} href={href} className="group p-6 border border-vtm-gray-border bg-white hover:border-vtm-dark transition-colors">
                <h3 className="font-headline font-bold text-vtm-dark mb-3 group-hover:text-vtm-red transition-colors">{name}</h3>
